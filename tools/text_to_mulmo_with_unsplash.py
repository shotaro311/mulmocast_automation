#!/usr/bin/env python3
"""
台本テキストをmulmocastスクリプト形式に自動変換するツール（Unsplash統合版）
使用方法: python text_to_mulmo_with_unsplash.py "台本テキスト" [output_file.json] [time_period] [--use-unsplash]
"""
...
def create_unsplash_image_config(image_path):
    """Unsplash画像の設定オブジェクトを作成"""
    return {
        "type": "image",
        "source": {
            "kind": "path",
...
def add_main_content_beats(script, sentences, use_unsplash=False, unsplash_fetcher=None):
    """本文のbeatsを追加"""
    for i, sentence in enumerate(sentences):
        if sentence.strip():
            if use_unsplash and unsplash_fetcher:
                # Unsplash画像を使用
                print(f"🎨 Unsplash画像選定中 ({i+1}/{len(sentences)}): {sentence[:40]}...")
                image_path = unsplash_fetcher.fetch_smart_image(sentence)
                
                if image_path:
                    image_config = create_unsplash_image_config(image_path)
                    beat = create_beat_config("Presenter", sentence, image_config=image_config)
                    print(f"✅ Unsplash画像を使用: {os.path.basename(image_path)}")
                else:
                    # Unsplash取得失敗時はプロンプトにフォールバック
                    image_prompt = generate_image_prompt(sentence)
                    beat = create_beat_config("Presenter", sentence, image_prompt=image_prompt)
                    print(f"⚠️  Unsplash取得失敗、プロンプト使用: {image_prompt[:50]}...")
            else:
                # 従来のプロンプト方式
                image_prompt = generate_image_prompt(sentence)
                beat = create_beat_config("Presenter", sentence, image_prompt=image_prompt)
...
def print_generation_summary(output_file, total_beats, fixed_images, use_unsplash=False):
    """生成結果のサマリーを出力"""
    ...
    if use_unsplash:
        print(f"🎨 画像取得: Unsplash API使用（高品質画像）")
    elif fixed_images:
...
def text_to_mulmo_script(text, output_file=None, time_period="morning", use_unsplash=False):
    ...
    # Unsplash設定
    unsplash_fetcher = None
    if use_unsplash:
        print("🎨 Unsplash統合モード: 高品質画像を使用します")
        try:
            unsplash_fetcher = UnsplashImageFetcher()
        except ValueError as e:
            print(f"❌ Unsplash設定エラー: {e}")
    ...
    # 2. 本文のbeats追加
    add_main_content_beats(script, sentences, use_unsplash, unsplash_fetcher)
    ...
    if output_file is None:
        ...
        prefix = "tech_unsplash" if use_unsplash else "tech"
    ...
    print_generation_summary(output_file, total_beats, fixed_images, use_unsplash)
    
    return output_file

def main():
    if len(sys.argv) < 2:
        print("使用方法: python text_to_mulmo_with_unsplash.py '台本テキスト' [output_file.json] [time_period] [--use-unsplash]")
        print("time_period: 'morning'(朝用・デフォルト) または 'evening'(夜用)")
        print("--use-unsplash: Unsplash APIで高品質画像を取得")
        sys.exit(1)
    ...
    result_file = text_to_mulmo_script(text, output_file, time_period, use_unsplash)
